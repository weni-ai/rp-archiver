...
func reCreateArchive(ctx context.Context, db *sqlx.DB, config *Config, s3Client s3iface.S3API, archive *Archive) error {
	err := CreateArchiveFile(ctx, db, archive, config.TempDir)
	if err != nil {
		return errors.Wrap(err, "error writing archive file")
	}
...
func reCreateArchives(ctx context.Context, db *sqlx.DB, config *Config, s3Client s3iface.S3API, org Org, archives []*Archive) error {
	log := logrus.WithFields(logrus.Fields{
		"org":    org.Name,
		"org_id": org.ID,
	})
	...
	for _, archive := range archives {
		...
		err := reCreateArchive(ctx, db, config, s3Client, archive)
		if err != nil {
			log.WithError(err).Error("error recreating archive")
			continue
		}
...
func ArchiveRollupOrgSingleMonth(ctx context.Context, db *sqlx.DB, config *Config, s3Client s3iface.S3API, org Org, year string, month string, archiveType ArchiveType) ([]*Archive, error) {
	...
	err = reCreateArchives(ctx, db, config, s3Client, org, dailies)
	if err != nil {
		return nil, err
	}
